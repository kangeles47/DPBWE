...
for idx in df_inventory.index.to_list():
    ...
    if df_inventory['YearBuilt'][idx] < 1950:
        year_range = 1
    elif 1950 <= df_inventory['YearBuilt'][idx] < 1960:
        year_range = 2
    elif 1960 <= df_inventory['YearBuilt'][idx] < 1970:
        year_range = 3
    elif 1970 <= df_inventory['YearBuilt'][idx] < 1980:
        year_range = 4
    elif 1980 <= df_inventory['YearBuilt'][idx] < 1990:
        year_range = 5
    elif 1990 <= df_inventory['YearBuilt'][idx] < 2000:
        year_range = 6
    elif 2000 <= df_inventory['YearBuilt'][idx] < 2010:
        year_range = 7
    elif 2010 <= df_inventory['YearBuilt'][idx] < 2016:
        year_range = 8
    elif 2016 <= df_inventory['YearBuilt'][idx] < 2021:
        year_range = 9
    # Query samples
    samples = df_recs.loc[(df_recs['DIVISION']==division) & (df_recs['state_postal']=='FL') & (df_recs['STORIES']==1) & (df_recs['YEARMADERANGE']==year_range)]
    if len(samples) > 0:
        garage_options = []
        garage_weights = []
        for u in samples['PRKGPLC1'].unique():
