...
def get_rshape(file_path, county, building_type, number_of_stories, roof_cover):
    df = pd.read_csv(file_path)
    df_sub = df.loc[(df['address_sub_admin_area'] == county.upper()) & (df['building_type'] == building_type) & (
...
df_inventory = pd.read_csv('D:/Users/Karen/Documents/GitHub/DPBWE/PCB_Building_Inventory.csv')
roof_shapes = []
seed(101)
for idx in df_inventory.index.to_list():
    shape_choice = get_rshape(file_path, 'BAY', 'Single Family', 1, 'Asphalt')
