...
def map_tap_data(tpu_wdir, model_file, num_surf, bfull, hfull, dfull, side_lines, surf_dict, wind_speed, match_flag, h_bldg, rect_surf_dict, bldg):
    ...
    # plt.plot(df['x'], df['y'], 'o')
    # plt.show()
    # Step 2: Attach Cp time history to each pressure tap location:
    tlabels_list = []
    for t in range(0, len(tpu_data['Wind_pressure_coefficients'][:, 0])):
        tlabels_list.append('t' + str(t))
    df_cp = pd.DataFrame(tpu_data['Wind_pressure_coefficients'].T, columns=tlabels_list)
    ...
    contour_values = {'x': [], 'y': [], 'Surface Number': []}
    ...
    for surf in range(1, num_surf + 1):
        if num_surf == 5:
            ...
            if surf == 1 or surf == 3:
                ...
                boundary_cps = {}
                for t in tlabels_list:
                    boundary_cps[t] = [df_surf[t][ind_list[1]], df_surf[t][ind_list[0]], df_surf[t][ind_list[2]], df_surf[t][ind_list[3]]]
                ...
                boundary_cps = {}
                for t in tlabels_list:
                    boundary_cps[t] = [df_surf[t][ind_list[0]], df_surf[t][ind_list[2]],
                                        df_surf[t][ind_list[3]], df_surf[t][ind_list[1]]]
            elif surf == 5:
            ...
            for pt in range(0, len(px)):
                new_tap_dict = {'x': px[pt], 'y': py[pt], 'Point Number': df['Point Number'].iloc[-1] + 1,
                                'Surface Number': 1 * surf}
                for key in boundary_cps.keys():
                    new_tap_dict[key] = boundary_cps[key][pt]
                df = df.append(new_tap_dict, ignore_index=True)
            del boundary_cps
            ...
            for x in index_list:
                if xcoords[x] == min_x:  # recall that this is original min_x
                    ...
                    new_tap_dict = {'x': min(df.loc[df['Surface Number'] == surf, 'x']), 'y': ycoords[x], 'Point Number': df['Point Number'].iloc[-1] + 1,
                                    'Surface Number': 1 * surf}
                elif xcoords[x] == max_x:
                    new_tap_dict = {'x': max(df.loc[df['Surface Number'] == surf, 'x']), 'y': ycoords[x], 'Point Number': df['Point Number'].iloc[-1] + 1,
                                    'Surface Number': 1 * surf}
                # Add Cp labels:
                for t in tlabels_list:
                    new_tap_dict[t] = df[t][x]
                df = df.append(new_tap_dict, ignore_index=True)
            # Loop through ycoords and create vertical boundaries:
            for y in index_list:
                if ycoords[y] == min_y:  # recall that this is original min_x
                    # Add new point to DataFrame and use current point's Cp:
                    new_tap_dict = {'x': xcoords[y], 'y': min(df.loc[df['Surface Number'] == surf, 'y']),
                                    'Point Number': df['Point Number'].iloc[-1] + 1,
                                    'Surface Number': 1 * surf}
                elif ycoords[y] == max_y:
                    new_tap_dict = {'x': xcoords[y], 'y': max(df.loc[df['Surface Number'] == surf, 'y']),
                                    'Point Number': df['Point Number'].iloc[-1] + 1,
                                    'Surface Number': 1 * surf}
                # Add Cp labels:
                for t in tlabels_list:
                    new_tap_dict[t] = df[t][y]
                df = df.append(new_tap_dict, ignore_index=True)
        else:
            pass
        ...
        # Time to find Cps across contours:
        for t in tlabels_list:
            # Extract the pressure coefficients for each surface:
            surf_cps = df.loc[df['Surface Number'] == surf, t]
            ...
            for col in range(0, len(xvals)):
                for row in range(0, len(yvals)):
                    ...
                    contour_values[t].append(zvals[row][col])
    ...
    if tpu_wdir > 90:
        ...
        for col in range(0, len(df_contour.columns)):
            if df_contour.columns[col] == 'x':
                ...
                pass
        for row in range(0, len(df_contour['Surface Number'])):
