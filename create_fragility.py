...
def execute_fragility_workflow(bldg, site, component_type, hazard_type, event_year, event_name, data_types, file_paths, damage_scale_name, analysis_date, hazard_file_path):
    ...
    if export_sample_flag:
        ...
        # Export as csv file:
        pd.DataFrame(sample_dict).to_csv('SampleBuildings_preFBC.csv', index=False)
    ...
    for ds in df_params['DS Number'].unique():
        ...
        if len(fail_unique) == 1 and fail_unique[0] == 0:
            ...
            pass
    df_params.to_csv('Observations_preFBC.csv', index=False)
...
def conduct_bayesian_norm(xj, zj, nj, mu_init, beta_init, num_samples=None, plot_flag=True):
    ...
    with pm.Model() as model:
        ...
        mu = BoundedNormal('mu', mu_init / norm_factor, 15 / norm_factor)
        ...
        trace = pm.sample(4000, cores=1, return_inferencedata=True, tune=2000, random_seed=52)
        ...
        if plot_flag:
            ...
            x = np.linspace(xmin, xmax, 100)
            p_prior = norm.pdf(x, mu_init/2.237, 15/2.237)
...
# observations_file_path = 'C:/Users/Karen/PycharmProjects/DPBWE/Observations_postFBC.csv'
# df = pd.read_csv(observations_file_path)
